...
public class WOASpringSessionRepository implements SessionRepository<ExpiringSession> {
	...
	public ExpiringSession getSession(String id) {
		...
		SessionEntity sessionEntity=sessionRep.getOne(id);
	...
	SessionEntity sessionEntity=new SessionEntity(session.getId(), session.getCreationTime(), session.getLastAccessedTime(), session.getMaxInactiveIntervalInSeconds(), (String)session.getAttribute("USERNAME"), session.isExpired());
